...
async function run() {
  try {
    ...
    const verifySeller = async (req, res, next) => {
      const email = req.tokenEmail
      const user = await usersCollection.findOne({ email })
      if (user?.role !== 'seller') return res.status(403).send({ message: 'Seller only Actions!', role: user?.role })
    ...
    app.post('/payment-succcess', async (req, res) => {
    ...
    app.get('/my-inventory/:email', verifyJWT, verifySeller, async (req, res) => {
      ...
      const result = await plantsCollection.find({
        'seller.email': email
      }).toArray()
      res.send(result)
    })
    ...
    app.post('/become-seller', verifyJWT, async (req, res) => {
      ...
      if (alreadyExists) return res.status(409).send({ message: 'Already requested, wait koro.' })
